...
Elm.Site.make = function (_elm) {
   ...
   var viewJobs = F3(function (address,fil,maybeJobs) {
      ...
      var sortIndicator = F2(function (s,f) {
         ...
         _U.eq(A2($Models.sortOnCriteria,f,jobs),jobs) ? "v " : _U.eq(A2($Models.sortOnCriteria,f,jobs),$List.reverse(jobs)) ? "^ " : "",
         s);
      });
      ...
      _U.list([A2($Html.th,
              ...
              _U.list([$Html.text(A2(sortIndicator,"Posted",$Models.PostingDate))]))
              ,A2($Html.th,
              _U.list([A2($Html$Events.onClick,address,$Models.SortJobs($Models.ClosingDate)),$Html$Attributes.$class("rightHead")]),
              _U.list([$Html.text(A2(sortIndicator,"Closing",$Models.ClosingDate))]))]))]),
      tbody));
   });

...
Elm.Site.make = function (_elm) {
   ...
   var viewJobs = F3(function (address,fil,maybeJobs) {
      ...
      var sortIndicator = F2(function (f,s) {
         return A2($Basics._op["++"],
         s,
         _U.eq(A2($Models.sortJobs,f,$Maybe.Just(jobs)),$Maybe.Just($List.reverse(jobs))) ? " v" : _U.eq(A2($Models.sortJobs,f,$Maybe.Just(jobs)),
         $Maybe.Just(jobs)) ? " ^" : "");
      });
...
Elm.Main.make = function (_elm) {
   ...
   var makeEntitiesVisible = F2(function (str,f) {
      var _p2 = str;
      if (_p2 === "") {
            return _U.update(f,{visibleOrgs: $Dict.keys(f.allOrgs),visibleRegions: $Dict.keys(f.allRegions)});
         } else {
            var ls = A2($String.split,",",_p2);
            return _U.update(f,
            {visibleOrgs: ls
            ,visibleRegions: $Set.toList($Set.fromList(A2($List.map,function (x) {    return A2($Maybe.withDefault,"",A2($Dict.get,x,f.allOrgs));},ls)))});
         }
   });
